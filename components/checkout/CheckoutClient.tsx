...
export default function CheckoutClient({ unit, user }: CheckoutClientProps) {
  ...
  const depositAmount = Math.round(unit.totalPrice * 0.15);
  const balanceAfterDeposit = unit.totalPrice - depositAmount;
  ...
  const {
    ...
    defaultValues: {
      ...
      email: user?.email || "",
      phone: user?.phone || "",
      idNumber: user?.nationalId || user?.personalNumber || "",
      paymentMethod: "mpesa",
    },
  });
  ...
  const onSubmit = async (data: CheckoutFormData) => {
    ...
    try {
      const result = await createBooking({
        unitId: unit.unitId,
        ...data,
        depositAmount, // calculated on client
      });
      if (result.success) {
        ...
        <div className="lg:col-span-2 order-1 lg:order-2">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6 md:p-8">
            ...
            <p className="text-gray-500 mb-6">
              Please confirm your details and choose a payment method
            </p>
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      type="text"
                      {...register("fullName")}
                      className="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 focus:ring-2 focus:ring-green-500 focus:border-transparent"
                    />
                    {errors.fullName && (
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm text-gray-700 dark:text-gray-300">
                      A deposit of <strong>{formatPrice(depositAmount)}</strong>{" "}
                      (15% of the total price) is required to secure this unit.
                    </p>
                    <p className="text-xs text-gray-500 mt-1">
                      The remaining balance of{" "}
                      {formatPrice(balanceAfterDeposit)} will be payable upon
